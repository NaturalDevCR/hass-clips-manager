...
def test_two_pass_loudness_runs_analysis_then_feeds_measured_values_to_encode(tmp_path):
    ...
    result = JobWorker(db, resolver, probe_client=_ValidProbe(), process_factory=process_factory).run_once()
    ...
    assert "measured_I=-19" in final_graph and "offset=1" in final_graph


def test_completed_job_is_not_enqueued_again_when_output_is_still_available(tmp_path):
    db, resolver, service = _configured_service(tmp_path)
    service.enqueue_compile(CompileRequest(collection_id="films"))
    ...
    assert JobWorker(db, resolver, probe_client=_ValidProbe(), process_factory=process_factory).run_once()
