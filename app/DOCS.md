...
The Library Manager is a single-page interface with four sections — Collections, Library, Import, and System. Collections and processing profiles are configured here; the Home Assistant integration reads them. It can import source clips, rename them within their selected collection, record tags and notes, request scans/recompiles, recover selected source/output files from Worker-owned trash, and permanently delete one explicitly selected catalog item after confirmation. Trash is not automatically purged. Deleting source does not delete compiled output, and vice versa.
